...
CircleAndRectangleDetector::CircleAndRectangleDetector(const std::string& fileName) {
    cv::Mat image = cv::imread(fileName, CV_LOAD_IMAGE_COLOR);
    cv::Mat imageGrayscale = cv::imread(fileName, CV_LOAD_IMAGE_GRAYSCALE);

    cv::blur(imageGrayscale, imageGrayscale, cv::Size(5, 5));

    cv::Canny(imageGrayscale, imageGrayscale, 170, 200);

    std::vector<std::vector<int> > lineAccumulator(180, std::vector<int>((image.rows + image.cols - 2) * 2, 0));

    accumulateLines(imageGrayscale, lineAccumulator);

    std::vector<std::pair<int, Line> > allLines;

    getAllLines(lineAccumulator, allLines, 140, image.rows, image.cols);
    ...
    std::vector<std::vector<int> > circleAccumulator(imageGrayscale.rows, std::vector<int>(imageGrayscale.cols, 0));

    int r = 63;

    accumulateCircles(imageGrayscale, circleAccumulator, r);
